...
from app.state import DEFAULT_STACK_POINTER, create_state, get_state, delete_state, GLOBAL_DICT
...
def test_create_state_single_mode_initializes_pipeline_buffers():
    state = create_state("user1", b"\x13\x00\x00\x00", mode="single")
    ...
    assert state["regs"][2] == DEFAULT_STACK_POINTER


def test_create_state_pipeline_mode_initializes_buffers():
    state = create_state("user2", b"\x13\x00\x00\x00", mode="pipeline")
...
def test_absolute_address_load_store():
    state = create_state("user3", b"\x13\x00\x00\x00", mode="single")
    state["dmem"][0x80001000] = 0x12345678
    assert state["dmem"][0x80001000] == 0x12345678


def test_get_and_delete_state():
    create_state("user4", b"\x13\x00\x00\x00", mode="single")
